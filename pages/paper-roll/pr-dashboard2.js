...
export default function AnalysisPage({ plant }) {
  ...
  return (
    <div style={{ padding: '1.5rem' }}>
      <h1 style={{ fontSize: '1.875rem', fontWeight: 'bold', marginBottom: '1.5rem' }}>Stock Age Analysis</h1>
      {error && <p style={{ color: '#ef4444' }}>Error: {error}</p>}
      <div style={styles.card}>
        <div style={styles.cardHeader}>
          <h2 style={styles.cardTitle}>Stock Age Summary</h2>
          ...
          <table style={styles.table}>
            <thead>
              <tr>
                ...
                <th style={{ ...styles.tableHead, ...styles.textRight }}>Number of Rolls</th>
                <th style={{ ...styles.tableHead, ...styles.textRight }}>Total Weight (kg)</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                <tr>
                  <td colSpan="3" style={{ ...styles.tableCell, textAlign: 'center' }}>Loading...</td>
                </tr>
              ) : (
                agingData.map(agingItem => (
                  <React.Fragment key={agingItem.key}>
                    <tr onClick={() => toggleAging(agingItem.key)} style={styles.cursorPointer}>
                      ...
                      <td style={{ ...styles.tableCell, ...styles.textRight, ...styles.fontMono }}>
                    ...
                    {expandedAging.has(agingItem.key) && Object.entries(agingItem.stats.kinds).sort(([,a], [,b]) => b.weight - a.weight).map(([kindName, kindStats]) => {
                      ...
                      return (
                        <React.Fragment key={kindKey}>
                          <tr onClick={() => toggleKind(kindKey)} style={{ ...styles.cursorPointer, ...styles.bgMuted(0.5) }}>
                            <td style={{ ...styles.tableCell, ...styles.pl(8) }}>
                              <span style={{ marginRight: '0.5rem' }}>
                                {isKindExpanded ? '▼' : '▶'}
                              </span>
                              {kindName}
                            </td>
                            <td style={{ ...styles.tableCell, ...styles.textRight, ...styles.fontMono }}>{kindStats.rolls.toLocaleString()}</td>
                            <td style={{ ...styles.tableCell, ...styles.textRight, ...styles.fontMono }}>{kindStats.weight.toLocaleString(undefined, { maximumFractionDigits: 0 })}</td>
                          </tr>
                          {isKindExpanded && Object.entries(kindStats.gsms).sort(([gsmA], [gsmB]) => Number(gsmB) - Number(gsmA)).map(([gsmValue, gsmStats]) => {
                            ...
                            return (
                              <React.Fragment key={gsmKey}>
                                <tr onClick={() => toggleGsm(gsmKey)} style={{ ...styles.cursorPointer, ...styles.bgMuted(0.3) }}>
                                  <td style={{ ...styles.tableCell, ...styles.pl(16) }}>
                                    <span style={{ marginRight: '0.5rem' }}>
                                      {isGsmExpanded ? '▼' : '▶'}
                                    </span>
                                    GSM: {gsmValue}
                                  </td>
                                  <td style={{ ...styles.tableCell, ...styles.textRight, ...styles.fontMono }}>{gsmStats.rolls.toLocaleString()}</td>
                                  <td style={{ ...styles.tableCell, ...styles.textRight, ...styles.fontMono }}>{gsmStats.weight.toLocaleString(undefined, { maximumFractionDigits: 0 })}</td>
                                </tr>
                                {isGsmExpanded && Object.entries(gsmStats.widths).sort(([widthA], [widthB]) => Number(widthB) - Number(widthA)).map(([widthValue, widthStats]) => (
                                  <tr
                                    ...
                                    style={{ ...styles.cursorPointer, ...styles.bgMuted(0.2) }}
                                  >
                                    <td style={{ ...styles.tableCell, ...styles.pl(24) }}>
                                      <span style={{ marginLeft: '1.5rem' }}>Width: {widthValue}</span>
                                    </td>
                                    <td style={{ ...styles.tableCell, ...styles.textRight, ...styles.fontMono }}>{widthStats.rolls.toLocaleString()}</td>
                                    <td style={{ ...styles.tableCell, ...styles.textRight, ...styles.fontMono }}>{widthStats.weight.toLocaleString(undefined, { maximumFractionDigits: 0 })}</td>
                                  </tr>
                                ))}
                              </React.Fragment>
                            )
